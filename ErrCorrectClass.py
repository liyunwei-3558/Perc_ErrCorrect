...
class Regressor:
    ...
    def visualize(self, Image=np.zeros((590, 1920, 3), np.uint8), colors=(0, 0, 255)):
        ...
        width = Image.shape[1]
        print("vis w=%d, h=%d" % (width, height))
        X = np.arange(1, width / self.w_resize, step=0.01)
        x = X.copy()
        X = X[:, np.newaxis]
        ...
        for i in np.arange(x.size):
            if Y[i] <= 0:
                continue
            cv2.circle(Image, (round(x[i] * self.w_resize), round(Y[i] * self.h_resize)), 6, (255, 255, 255))

        # Plot origin points
        for i in np.arange(self.x_lst.size):
            cv2.circle(Image, (round(self.x_lst[i] * self.w_resize), round(self.t_lst[i] * self.h_resize)), 5,
                       colors, 2)
    ...
    def get_real_W(self):
        ...
        for i in range(self.M+1):
            al.append(self.h_resize**(-1*i))
        A_re = np.array(al)
        A_re = np.diag(A_re)

        beta = self.w_resize
        # print(A_re)
        return beta * np.matmul(A_re, self.w)
